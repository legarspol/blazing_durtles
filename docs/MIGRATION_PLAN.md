...
- **Feature folders inside each layer.** New work groups by feature *within* a layer, not across layers: `…/ui/onboarding/`, `…/data/onboarding/`, `…/domain/onboarding/`. The layer stays the outer term because it is what the module boundary enforces — `:core` is a separate Gradle module, so a single vertical `feature/onboarding/` holding its own `ui`/`data`/`domain` could never contain the domain half anyway. Shared UI machinery that is not a feature (`…/ui/theme/`) stays where it is. Onboarding is the first of these and the reference shape.

- **One Compose host activity.** Migrated screens are hosted by a single Compose `Activity` with a Navigation graph, not one `Activity` per screen. `…/ui/onboarding/OnboardingActivity` is that host today; it deliberately does not extend `AbstractActivity`, which is DELETE-bound (`AUDIT.md` §1).
...
- **The API token stays in `EncryptedSharedPreferences`; the move off it is deferred (decided on #62).** DataStore has no encrypted variant and `androidx.security.crypto` has no maintained successor, so there is nowhere better to move the token *to* — and moving it is cheap whenever we want: read it from the old store, write it to the new one, once. Deferring costs nothing. What was worth doing now was deleting the **three generations** of storage `EncryptedPreferenceDataStore` used to read through (encrypted store → plain prefs with an `"enc:"` prefix and a hardcoded AES key → plain cleartext); generations 2 and 3 existed only to migrate Flaming Durtles installs we do not have, and carried a compiled-in symmetric key with a fixed zero IV. So `:app` keeps **two** preference stores for now: `EncryptedSharedPreferences` for the token and website password, DataStore for everything else (#61).
- **The Java-interop shims in ported Kotlin are scheduled debt, not house style.** Where surviving Java callers bind to a name, the port preserves that name rather than editing out-of-scope call sites: `@JvmField` on entity properties (`SubjectEntity`, `LogRecord`, `SearchPreset`, `SrsSystemDefinition`), `@get:JvmName("isQuestionNDone")` on `SessionItem`, and `get`-prefixed *functions* instead of Kotlin properties (`PropertiesDao.getIsMuted()`, `getCurrentQuestionType()`, `LogRecordDao.getTotalSize()`). Each exists solely to keep a Java call site compiling. **Do not imitate this shape in new Kotlin, and delete each shim as its last Java caller converts** — by the end of Phase 3 none should remain.
